...
describe EbayrBusinessPolicies::Request do

  describe "serializing input" do
    it "converts times" do
      result = EbayrBusinessPolicies::Request.serialize_input(Time.utc(2010, 'oct', 31, 03, 15))
  ...
  describe "uri" do
    it "is the Ebayr one" do
      EbayrBusinessPolicies::Request.new(:Blah).uri.must_equal(EbayrBusinessPolicies.uri)
    end
  end
  ...
  describe "arrays" do
    it "converts multiple arguments in new function" do
      args = [{ :a => 1 }, { :a => [{:b => 1 }, { :b => 2 }] }]
      EbayrBusinessPolicies::Request.new(:Blah, :input => args).input_xml.must_equal '<a>1</a><a><b>1</b><b>2</b></a>'
    end

    it "converts times" do
      args = [{ :Time => Time.utc(2010, 'oct', 31, 03, 15)}]
      result = EbayrBusinessPolicies::Request.new(:Blah, args).input_xml
  ...
  describe "xml" do
    def request(*args)
      EbayrBusinessPolicies::Request.xml(*args)
    end
    ...
    describe "requester credentials" do
      it 'includes requester credentials when auth_token present' do
        ...
        request = EbayrBusinessPolicies::Request.new(:Blah, :auth_token => my_token)
      ...
      it 'excludes requester credentials when auth_token not present' do
        request = EbayrBusinessPolicies::Request.new(:Blah, :auth_token => nil)
